...
st.title("📄 Automated PDF Report Generator")
st.markdown("Generate executive, publication-ready PDF analytics reports with 1-click — save templates or batch-generate for every dataset.")
...
if st.button("🚀 Compile & Generate PDF Report", type="primary"):
    with st.spinner("Compiling PDF tables, statistics, and metadata..."):
        try:
            pdf_filename, pdf_bytes = _build_report(selected_file, df, rep_title, author, include_charts)
            st.session_state["last_pdf_report"] = {
                "dataset": selected_file,
                "filename": pdf_filename,
                "bytes": pdf_bytes,
            }
            st.success(f"🎉 PDF Report successfully generated and saved to `Reports/{pdf_filename}`!")
        except Exception as e:
...
if st.button("🗂️ Generate Reports for ALL Datasets"):
